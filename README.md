...
| 本人ページ `/enroll/[token]` | **トップ / メッセージ / 設定**の 3 タブ。ホーム画面に追加するとアプリとして使えます。 |

本人ページのタブは、**トップ**（未確認の件数と一覧）・**メッセージ**（届いた連絡の一覧）・
**設定**（通知の設定とお名前の変更）です。
**パソコンでは左に大きなメニュー、右に未確認の連絡・通知の状態・時計**を並べた 3 段組みになります（スマートフォンでは上部のタブ）。
**画面を開いたままでも新しい連絡は自動で表示されます**（通知の受信時に即時／30 秒ごと／画面に戻った時点。「更新」ボタンもあります）。
